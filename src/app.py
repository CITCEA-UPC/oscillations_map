...
This project is now a static site deployed via GitHub Pages.
The static version lives in the docs/ directory.

To run this legacy Flask server for local development:
...
def download_excel():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for i, col in enumerate(df.columns):
            content_max_length = df[col].astype(str).apply(len).max() + 1
            title_length = len(col) + 6
            optimal_width = max(content_max_length, title_length)
            worksheet.set_column(i, i, optimal_width)
            worksheet.write(0, i, col.capitalize(), header_format)
...
if __name__ == "__main__":
    port_flask = int(os.getenv("FLASK_RUN_PORT", 8080))
    host_flask = os.getenv("FLASK_RUN_HOST", "0.0.0.0")
    ssl_context = (
